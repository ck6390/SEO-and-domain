...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype._create = function () {
        ...
        this.element.append('<input placeholder="Eg - Admin@123" name="newPassword" id="newPassword" type="password" required class="' + options.inputClass + '" "/>');
        ...
        this.element.append(
            ...
            <li data-criterion="sChars" class="valid">At least <strong>one Special Characters</strong></li> \
    ...
    Plugin.prototype._bind_input_events = function($s) {
        ...
        $s.bind('keyup', function () {
            ...
            var sChar = password.match(speacilChar) ? 1 : 0;
            var containWhiteSpace = password.indexOf(' ') >= 0 ? 1 : 0;

            var total = characters + capitalletters + loweletters + number + sChar;
            ...
            that._update_info('sChars', sChar);
    ...
    Plugin.prototype._update_indicator = function(total) {
        ...
        if (total === 0) {
            ...
            meter.addClass('veryweak').html('<p class="text-center">very weak</p>');
        } else if (total === 2) {
            meter.addClass('weak').html('<p class="text-center">weak</p>');
        } else if (total === 3) {
            meter.addClass('medium').html('<p class="text-center">medium</p>');
        } else {
            meter.addClass('strong').html('<p class="text-center">very strong</p>');
        }
    },
